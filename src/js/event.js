...
import { ITEMS_PER_PAGE, formatAuthorLabel, getByteLength, linkifyText, isSafeAttachmentData, MAX_ATTACHMENT_FILE_BYTES, uiIcon, iconLabel } from './utils.js';
...
export async function addEvent() {
    ...
    try {
        ...
        // 파일 업로드·본문은 부모 카드가 커밋된 뒤에 해야 한다 — storage.rules와
        // 본문 규칙 둘 다 부모 문서를 get()으로 읽어 작성자를 확인하므로, 카드가
        // 아직 없으면 업로드도 본문 쓰기도 거부된다.
        ...
        for (const file of selectedFiles) {
            const path = `attachments/events/${eventRef.id}/${crypto.randomUUID()}`;
...
// URI)를 둘 다 처리한다. Storage 다운로드 URL은 다른 오리진이라 <a download>가
// 무시될 수 있어 fetch로 받아 blob: URL을 만들어 원래 파일명으로 저장한다.
...
export async function deleteCurrentEvent() {
    ...
    try {
        ...
        const contentSnap = await getDoc(doc(db, 'events', currentEventDocId, 'content', 'main')).catch(() => null);
