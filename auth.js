...
let __authStatusTimer = null;
function setAuthStatus(message, type = 'info', durationMs = undefined) {
    ...
    if (!message) {
        ...
        if (__authStatusTimer) {
            clearTimeout(__authStatusTimer);
            __authStatusTimer = null;
        }
        return;
    }

    // 기본 지속시간: success 4s, error 7s, info 3s
    const defaultDurations = { success: 4000, error: 7000, info: 3000 };
    const timeout = typeof durationMs === 'number' ? durationMs : (defaultDurations[type] || 3000);
...
export function initializeAuthCallbacks(callback) {
    ...
    onAuthStateChanged(auth, async (user) => {
        try {
            console.log('[Auth] onAuthStateChanged triggered', { user: user ? { uid: user.uid, email: user.email, isAnonymous: user.isAnonymous } : null });
            if (!applyUserSessionUIFunc) {
                console.warn('[Auth] applyUserSessionUIFunc is not set yet');
                return;
            }
            ...
            try {
                ...
                console.error('[Auth State] Failed to create signup profile:', err);
                clearPendingSignupData();
                clearPendingAuthIntent();
                setAuthStatus('회원가입 처리 중 오류가 발생했습니다. 콘솔을 확인해 주세요.', 'error');
        ...
        try {
            ...
            console.error('[Auth State] Failed to create login-only profile:', err);
            clearPendingAuthIntent();
            setAuthStatus('로그인 처리 중 오류가 발생했습니다. 콘솔을 확인해 주세요.', 'error');
...
window.showSignupPreview = async function(id, batch, name, provider) {
    ...
    try {
        ...
        if (err?.code === 'permission-denied') {
            console.warn('[Signup Preview] Duplicate check skipped due to insufficient permissions. This is expected for unauthenticated preview checks.');
            duplicateWarnings.push('⚠️ 현재 중복 검사에 필요한 권한이 없어 미리보기 중 중복 결과를 표시할 수 없습니다. 실제 가입 시에 중복 여부를 다시 확인합니다.');
        } else {
            console.error('[Signup Preview] Error checking duplicates:', err);
            duplicateWarnings.push('⚠️ 중복 검사 중 오류가 발생했습니다. 나중에 다시 시도해 주세요.');
        }
    }
