...
function listenAdminUserConsole() {
    ...
    onSnapshot(collection(db, 'users'), (snapshot) => {
        ...
        snapshot.forEach((docSnap) => {
            ...
            confirmBtn.addEventListener('click', () => commitRoleChange(u.id, roleSelect.value));
            ...
            warnBtn.addEventListener('click', () => warnUser(u.id));

            const deleteBtn = document.createElement('button');
            deleteBtn.type = 'button';
            deleteBtn.className = 'btn-mini btn-del';
            deleteBtn.textContent = '계정 삭제';
            deleteBtn.addEventListener('click', () => deleteUserByAdmin(u.id));
...
async function warnUser(userId) {
    ...
    await updateDoc(doc(db, 'users', userId), { warnings: increment(1) });
    alert('경고가 부여되었습니다.');
}

async function deleteUserByAdmin(userId) {
    if (!ensureAdminAction()) return;
    if (!confirm('⚠️ 이 유저를 강제 탈퇴시겠습니까? 삭제한 이후 복구가 불가합니다.')) return;
