...
async function addNotice() {
    ...
    if (needsNoticeTrafficCheck) {
        ...
        const noticeTrafficResult = await checkTrafficAllowed(loggedInUser.uid, 'noticeCount', 1, 5);
    ...
    if (fileInput?.files.length > 0) {
        ...
        if (needsUploadTrafficCheck) {
            ...
            const uploadTrafficResult = await checkTrafficAllowed(loggedInUser.uid, 'uploadBytes', totalNewSize, uploadLimit);
...
async function addComment() {
    ...
    if (loggedInUser.role !== 'admin') {
        ...
        const commentTrafficResult = await checkTrafficAllowed(loggedInUser.uid, 'commentCount', 1, 10);
...
async function addFaqQuestion() {
    ...
    const faqTrafficResult = await checkTrafficAllowed(loggedInUser.uid, 'faqQuestionCount', 1, 1);
...
function renderAdminUserConsole() {
